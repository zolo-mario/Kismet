...
void FBlueprintEditor::OnCollapseSelectionToFunction()
{
	...
	for (FGraphPanelSelectionSet::TConstIterator NodeIt(SelectedNodes); NodeIt; ++NodeIt)
	{
		if (UEdGraphNode* SelectedNode = Cast<UEdGraphNode>(*NodeIt))
		{
			...
			{
				CollapsableNodes.Add(SelectedNode);
				if (SelectedNode->Pins.ContainsByPredicate(IsExecPin))
					bPure = false;
			}
		}
	}
	...
	if (CollapsableNodes.Num() && CanCollapseSelectionToFunction(CollapsableNodes))
	{
		...
		if (bPure)
		{
			...
			{
				EntryNode->Modify();
				const uint32 CurrentFlags = EntryNode->GetExtraFlags();
				if ((CurrentFlags & FUNC_BlueprintPure) == 0)
				{
					EntryNode->SetExtraFlags(CurrentFlags | FUNC_BlueprintPure);
