...
public:
	...
	virtual void Compile(FKismetFunctionContext& Context, UEdGraphNode* Node) override
	{
		...
		if ((BoolTerm != NULL) && (SwitchSelectionTerm != NULL))
		{
			...
			// ++Zvx
			FBlueprintCompiledStatement& SwitchStatement = Context.AppendZvxStatementForNode(Node);
			SwitchStatement.ZvxTerminalMap.Add(TEXT("SwitchSelection"), SwitchSelectionTerm);
			// --Zvx
			...
			for (auto PinIt = SwitchNode->Pins.CreateIterator(); PinIt; ++PinIt)
			{
				...
				if ((Pin->Direction == EGPD_Output) && (Pin != DefaultPin) && (!bCanSkipUnlinkedCase || Pin->LinkedTo.Num() > 0))
				{
					...
					// ++Zvx
					SwitchStatement.ZvxTerminalList.Add(CaseValueTerm);
					SwitchStatement.ZvxStatementList.Add(&IfFailTest_SucceedAtBeingEqualGoto);
					// --Zvx
				}
			}

			// Finally output default pin
			FBlueprintCompiledStatement& DefaultStatement = GenerateSimpleThenGoto(Context, *SwitchNode, DefaultPin);
			// ++Zvx
			SwitchStatement.ZvxStatementList.Add(&DefaultStatement);
			// --Zvx
		}
		else
		{
