	...
	virtual void Compile(FKismetFunctionContext& Context, UEdGraphNode* Node) override
	{
		...
		if (OutputPins.Num() > 0)
		{
			if (Context.IsDebuggingOrInstrumentationRequired() && (OutputPins.Num() > 1))
			{
				...
				// ++Zvx
				FBlueprintCompiledStatement& ExecutionSequenceStatement = Context.AppendZvxStatementForNode(Node);
				// --Zvx
				...
				for (int32 i = 0; i < OutputPins.Num(); ++i)
				{
					...
					// ++Zvx
					ExecutionSequenceStatement.ZvxStatementList.Add(&GotoSequenceLinkedState);
					// --Zvx
				}
